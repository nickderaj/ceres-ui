...
A dark-themed React component library extracted from the [Ceres](https://github.com/nickderaj/ceres) blog. Ships with Catppuccin Mocha (dark) and Latte (light) themes, plain CSS with overridable custom properties, and zero Tailwind dependency.
...
<Chart
  data={{
    ...
    referenceLines: [
      { y: 2000, label: 'Support', color: '#a6e3a1', strokeDasharray: '6 3' },
    ],
  }}
...
ceres-ui/
  src/
    components/
      Callout/            Alert boxes (7 variants)
      CodeBlock/          Syntax-highlighted code
      TableOfContents/    Section navigation
      PostCard/           Post preview card
      DataTable/          Themed HTML table
      Chart/              Line charts & histograms (recharts)
      BackToTopButton/    Scroll-to-top (desktop)
      StarField/          Animated star background
      Badge/              Status indicators
      PdfDownloadButton/  Print-to-PDF trigger
      MarkdownRenderer/   Full markdown renderer
    styles/
      tokens.css          CSS custom properties (Mocha + Latte)
      base.css            Scrollbars, animations, resets
      catppuccin-mocha.ts Prism syntax theme object
    types/
      index.ts            All shared interfaces
    index.ts              Barrel export
  scripts/
    auto-version.mjs      Pre-commit version bumper
  test/
